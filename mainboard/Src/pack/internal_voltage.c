...
HAL_StatusTypeDef internal_voltage_measure() {
    ...
    ERROR_TOGGLE_CHECK_STR(fabsf(CONVERT_VALUE_TO_INTERNAL_VOLTAGE(internal_voltages.bat) - CONVERT_VALUE_TO_VOLTAGE(cell_voltage_get_sum())) > INTERNAL_VOLTAGE_MAX_DELTA, ERROR_VOLTAGE_MISMATCH, error_mainboard_instance);
    return HAL_OK;
}
