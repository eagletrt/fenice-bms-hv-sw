...
char *command_names[N_COMMANDS] = {"volt", "temp",  "status", "errors", "ts",
                                   "bal",  "dmesg", "reset",  "?",      "\ta"};

cli_command_func_t *commands[N_COMMANDS] = {
    &_cli_volts,   &_cli_temps, &_cli_status, &_cli_errors, &_cli_ts,
    &_cli_balance, &_cli_dmesg, &_cli_reset,  &_cli_help,   &_cli_taba};
...
void _cli_volts(uint16_t argc, char **argv, char *out) {
  if (strcmp(argv[1], "") == 0) {
    ...
    sprintf(out,
            ...
            (float)max / 10000, (float)min / 10000, (float)(max - min) / 10000);
...
#define n_items 4
  ...
  const char *values[n_items][2] = {
      {"BMS state", state_names[fsm_bms.current_state]},
...
void _cli_errors(uint16_t argc, char **argv, char *out) {
  ...
  for (uint16_t i = 0; i < count; i++) {
    sprintf(out + strlen(out),
            ...
            "offset......%u\r\n"
            "state.......%u\r\n",
            errors[i].id, error_names[errors[i].id], errors[i].timestamp,
            now - errors[i].timestamp, errors[i].offset, errors[i].state);
  }
}
