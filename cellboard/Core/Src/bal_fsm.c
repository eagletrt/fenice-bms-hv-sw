...
bal_fsm_transition_request set_bal_request = {.is_new = false, .next_state = STATE_OFF};
...
bal_state_t do_init(state_data_t *data) {
    ...
    switch (next_state) {
        ...
        default:
            sprintf(
                debug_msg, "[FSM] Cannot pass from init to %s, remaining in this state\r\n", state_names[next_state]);
...
bal_state_t do_discharge(state_data_t *data) {
    ...
    size_t count = bal_get_cells_to_discharge(volt_get_volts(), NULL, bal_params.target, bal_params.threshold);
...
void start_discharge(state_data_t *data) {
    ...
    bal_get_cells_to_discharge(volt_get_volts(), &bal_params.discharge_cells, bal_params.target, bal_params.threshold);
