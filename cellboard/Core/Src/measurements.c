...
void measurements_flags_check() {
    ...
    if (flags & MEASUREMENTS_VOLTS_READ_FLAG) {
        if (open_wire_check_status) {
            ...
            volt_read();
            can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_VOLTAGES);
            can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_VOLTAGES_INFO);
        }
    ...
    if (flags & MEASUREMENTS_TEMPS_READ_FLAG) {
        temp_measure_all();
        can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_TEMPERATURES);
        can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_TEMPERATURES_INFO);
        can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_BOARD_STATUS);
        can_send(CAN_BMS_MESSAGE_FRAME_ID_CELLBOARD_VERSION);
        // can_send(0);
...
void measurements_oc_handler(TIM_HandleTypeDef *htim) {
    ...
    switch (htim->Channel) {
        ...
        case VOLTS_READ_ACTIVE_CHANNEL:
            ...
            __HAL_TIM_SetCompare(
                htim, VOLTS_READ_CHANNEL, cnt + TIM_MS_TO_TICKS(htim, VOLT_MEASURE_INTERVAL + cellboard_index));
            flags |= MEASUREMENTS_VOLTS_READ_FLAG;
            break;

        case TEMPS_READ_ACTIVE_CHANNEL:
            __HAL_TIM_SetCompare(
                htim, TEMPS_READ_CHANNEL, cnt + TIM_MS_TO_TICKS(htim, TEMP_MEASURE_INTERVAL + cellboard_index));
            flags |= MEASUREMENTS_TEMPS_READ_FLAG;
            break;
