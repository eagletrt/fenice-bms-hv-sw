...
/** @brief Convert first hall sensor voltage measure to current value */
current_t _current_convert_low(float volt) {
    return (499.f / 300.f / 40e-3f) * (volt - V0L);
}
/** @brief Convert second hall sensor voltage measure to current value */
current_t _current_convert_high(float volt) {
    return (499.f / 300.f / 6.67e-3f) * (volt - V0H);
}
/** @brief Convert shunt voltage measure to current value */
current_t _current_convert_shunt(float volt) {
    return (volt - 0.468205124) / (1e-4f * 500);
}

// TODO: Read from hall-sensors channels
void current_start_measure() {
    HAL_ADC_Start_DMA(&ADC, (uint32_t *)adc_50, MEASURE_SAMPLE_SIZE);
    HAL_ADC_Start_DMA(&ADC, (uint32_t *)adc_300, MEASURE_SAMPLE_SIZE);
}

uint32_t current_read(float shunt_adc_val) {
    uint32_t time = HAL_GetTick();
    uint32_t avg_50 = 0, avg_300 = 0;
...
void current_check_errors() {
    ERROR_TOGGLE(current[CURRENT_SENSOR_300] > PACK_MAX_CURRENT, ERROR_OVER_CURRENT, 0);
}
