...
void ltc6804_adax(SPI_HandleTypeDef *hspi1)
	{
	    uint8_t cmd[4];
	    uint16_t cmd_pec;
...
//		101 GPIO 5
          ...
          //  	cmd[1] = (uint8_t)0x60 +chg; //0110 0010
	            cmd[1] = 0x62; //gpio2
	            cmd_pec = pec15(2, cmd,crcTable);
...
//GPI0= 1,2,3
void ltc6804_rdauxA(uint8_t ic_n, uint16_t aux_codes[5],SPI_HandleTypeDef *hspi1){
	...
	//	if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 + data[7])){
			...
			aux_codes[0] = convert_voltage(data);
			aux_codes[1] = convert_voltage(&data[2]);
			aux_codes[2] = convert_voltage(&data[4]);
//			aux_codes[3] = ;
//			aux_codes[4] = 0;
//			aux_codes[5] = 0;
