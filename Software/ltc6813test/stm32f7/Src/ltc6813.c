...
void ltc6813_rdcv_voltages(uint8_t TOT_IC, uint16_t cell_voltages[36][2],
						   SPI_HandleTypeDef *hspi1) {
	// address com format non pi� possible
	...
	for (reg = 0; reg < 6; reg++) {
		...
		HAL_SPI_Receive(hspi1, data, DATA_SIZE * TOT_IC,
						100);  // TODO   data8 checkpec16(data8) data8
							   // checkpec16(data8) dat8 pec16
		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
		// data0-5 volt, data6-7 pec
		for (int ic_n = 0; ic_n < TOT_IC; ic_n++) {
			uint8_t shift8 = ic_n * DATA_SIZE;

			if (pec15(6, data + shift8, crcTable) ==
				(uint16_t)(data[6 + shift8] * 256 + data[7 + shift8])) {
				cell_voltages[reg * CELL_IN_REG + ic_n * N_CELL_IN_STACK][0] =
					convert_voltage(&data[shift8]);
				cell_voltages[reg * CELL_IN_REG + ic_n * N_CELL_IN_STACK + 1]
							 [0] = convert_voltage(&data[2 + shift8]);
				cell_voltages[reg * CELL_IN_REG + ic_n * N_CELL_IN_STACK + 2]
							 [0] = convert_voltage(&data[4 + shift8]);
...
// analogo a rdcv
void ltc6813_rdaux(uint8_t TOT_IC, uint16_t gpio_voltages[9][2],
				   SPI_HandleTypeDef *hspi1) {
	uint8_t cmd[4];
	uint16_t cmd_pec;
	uint8_t data[8 * TOT_IC];
	cmd[0] = (uint8_t)0x00;
	wakeup_idle(hspi1);
	uint8_t reg;  // rdAUX A B C D

	uint8_t N_GPIO_IN_STACK = 18;
	uint8_t GPIO_IN_REG = 3;  // num of GPIOS returned by rdAUX
	uint8_t DATA_SIZE = 8;
	...
	for (reg = 0 + 6; reg < 4 + 6; reg++) {
		...
		for (int ic_n = 0; ic_n < TOT_IC; ic_n++) {
			...
			if (pec15(6, data + shift8, crcTable) ==
				(uint16_t)(data[6 + shift8] * 256 + data[7 + shift8])) {
				gpio_voltages[reg * GPIO_IN_REG + ic_n * GPIO_IN_REG][0] =
					convert_voltage(&data[shift8]);
				gpio_voltages[reg * GPIO_IN_REG + ic_n * GPIO_IN_REG + 1][0] =
					convert_voltage(&data[2 + shift8]);
				gpio_voltages[reg * GPIO_IN_REG + ic_n * GPIO_IN_REG + 2][0] =
					convert_voltage(&data[4 + shift8]);
...
//						HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
//
//						if(pec15(6, data, crcTable) == (uint16_t) (data[6]*256 +
//data[7])){
//
//							gpio_voltages[ic_n*9+9][0] = convert_voltage(data);
////							gpio_voltages[ic_n*9+7][0] =
///convert_voltage(&data[2]);
////							gpio_voltages[ic_n*9+8][0] =
...
void LTC6804s_I2CMUX(SPI_HandleTypeDef *hspi1, uint8_t ch) {
	...
	for (uint8_t current_byte = 0; current_byte < BYTES_IN_REG;
		 current_byte++)  // executes for each of the 6 bytes in the CFGR
						  // register
	{
		// current_byte is the byte counter

		cmd[cmd_index] = config[current_byte];  // adding the config data to the
												// array to be sent
		cmd_index = cmd_index + 1;
	}
	// 3
	cfg_pec = (uint16_t)pec15(BYTES_IN_REG, config,
							  crcTable);  // calculating the PEC for each ICs
										  // configuration register data
	cmd[cmd_index] = (uint8_t)(cfg_pec >> 8);
	cmd[cmd_index + 1] = (uint8_t)cfg_pec;
	cmd_index = cmd_index + 2;
	wakeup_idle(hspi1);  // This will guarantee that the LTC6804 isoSPI port is
...
void set_balancing(uint16_t cell_voltages[18][2], SPI_HandleTypeDef *hspi1,
				   uint16_t dcc_b[18], int dcto, uint16_t dcto_b[16]) {
	...
	if (highest.v > lowest.v + CELL_MARGIN) {
		ltc6813_DischargeCell_Enable(hspi1, highest.i, dcc_b[highest.i], dcto,
									 dcto_b[dcto]);
	}
}

void ltc6813_DischargeCell_Enable(SPI_HandleTypeDef *hspi1, int ndcc,
								  uint16_t dcc_b[18], int dcto,
								  uint16_t dcto_b[16]) {
	uint8_t cmd[4];
	uint8_t cfgr[8];
	uint16_t cmd_pec;
	...
	if (dcc < 8) {
		...
		cmd[1] =
			0x04;  // WRCFGB for dtmen       every 30 sec perform cell voltage
				   // conversion-> posso fare rdcv ogni 30 sec per monitorare
		cfgr[4] = 0x00;
		cfgr[5] = 0x00;
